...
def main(
    ...
    base_url: Optional[str] = typer.Option(None, help="The base URL for the LLM server (for providers like ollama)."),
    api_key: Optional[str] = typer.Option(None, help="The API key for the LLM provider (for providers like openai)."),
    ...
    if workflow == "planner":
        ...
        plan_task = BasicTask(
            description=f"Create a step-by-step plan to answer the user's query: '{query}'",
            expected_output="A list of tasks in a structured format.",
            agent=planner
        )
        ...
        plan_task = BasicTask(
            description=f"Create a detailed plan for full RAG workflow for query: '{query}'",
            expected_output="A structured list of analysis, and persistence steps.",
            agent=planner
        )
        intelligence_task = BasicTask(
            description=f"Analyze and synthesize information into an intelligence report for query: '{query}'",
            expected_output="A structured intelligence report.",
        ...
        tasks.extend([
            plan_task,
            intelligence_task,
            knowledge_persistence_task,
            retrieval_and_answer_task
        ])
