...
def test_main_cli_with_full_rag_workflow(
    ...
    mock_intelligence_instance = mock_intelligence_agent_cls.return_value
    mock_knowledge_instance = mock_knowledge_agent_cls.return_value
    mock_retrieval_instance = mock_retrieval_agent_cls.return_value
    ...
    # Tasks mocks
    mock_plan_task_instance = mock_basic_task_cls.return_value
    # Removed: mock_research_task_instance = mock_basic_task_cls.return_value
    mock_intelligence_task_instance = mock_basic_task_cls.return_value
    mock_knowledge_task_instance = mock_basic_task_cls.return_value
    mock_retrieval_task_instance = mock_basic_task_cls.return_value
    ...
    mock_knowledge_agent_cls.assert_called_once_with(
        llm=mock_llm_instance,
        qdrant_tool=mock_qdrant_tool_instance,
        outline_tool=mock_outline_tool_instance
    )
    mock_retrieval_agent_cls.assert_called_once_with(
        llm=mock_llm_instance,
        qdrant_tool=mock_qdrant_tool_instance
    )
    ...
    mock_orchestrator_cls.assert_called_once()
    mock_orchestrator_instance.add_agent.call_count == 4 # Reduced from 5
    mock_orchestrator_instance.add_task.call_count == 4 # Reduced from 5
