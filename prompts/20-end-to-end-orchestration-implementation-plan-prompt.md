...
1. **Goal and Vision:** The plan must define the `CrewAIManager` as the central orchestrator that manages a crew of specialized agents. The `Runner` will be the entry point that kicks off the process. The overall goal is
...
2. **Core Components and Data Models:**
    * **`agents-orchestrator-crewai`:**
        * `CrewAIManager`: The main class that will define the crew (agents and tasks) and manage the execution flow.
        * `WorkflowConfig`: A Pydantic model to configure the crew, including defining the agents and the sequence of tasks.
    * **`runner`:**
        * `Runner`: A simple class or function that will instantiate the `CrewAIManager` and all the necessary agents and tools, and then start the crew's execution.
    * **Input/Output:**
...
3. **High-Level Workflow:** The plan must detail the end-to-end workflow:
    ...
    4. The `Runner` instantiates the `CrewAIManager` with the configured crew.
    5. The `Runner` invokes the `CrewAIManager` with the user query.
    6. The `CrewAIManager` (leveraging `crewai`'s capabilities) executes the defined tasks in sequence, passing context between agents.
    ...
    7. After the crew finishes its work, the `CrewAIManager` returns the final result.
...
4. **TDD Implementation Cycle:** The plan must detail a RED-GREEN-REFACTOR cycle for the new packages.
    * **`agents-orchestrator-crewai` package:**
        ...
        * **Unit Tests:**
            * Test the `CrewAIManager`'s ability to be instantiated.
            * Use **mock agents** and **mock tools** to verify that the `CrewAIManager` correctly initializes the `crewai` `Crew` object based on a `WorkflowConfig`.
